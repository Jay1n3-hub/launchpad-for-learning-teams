...
CREATE TYPE public.app_role AS ENUM (
  'intern', 'dev', 'ba', 'mentor', 'scrum_master',
  'product_owner', 'pm', 'admin', 'ceo',
  'solutions_architect', 'designer'
);
...
CREATE POLICY "Authenticated users can view their org"
...
-- Get org by code RPC
CREATE OR REPLACE FUNCTION public.get_org_by_code(_code TEXT)
...
SET search_path = public
AS $$
  SELECT id, name FROM public.organizations WHERE code = _code
$$;
...
BEGIN
  ...
  IF _org_name IS NOT NULL AND _org_name != '' THEN
    ...
    VALUES (_org_name, public.generate_org_code())
    RETURNING id INTO _org_id;

    INSERT INTO public.user_roles (user_id, role)
    VALUES (NEW.id, 'admin');
  ELSIF _join_org_id IS NOT NULL THEN
    _org_id := _join_org_id;
  END IF;

  INSERT INTO public.profiles (user_id, org_id, name, email)
  VALUES (
    NEW.id,
    _org_id,
